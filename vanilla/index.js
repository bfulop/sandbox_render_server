...
const InvalidArguments = 'InvalidArguments';
...
function sendPage() {
    return pipe(H.status(H.Status.OK), H.ichain(() => H.closeHeaders()), H.ichain(() => H.send(`hello `)));
}
/** Parses the `user_id` param */
const getUserId = pipe(H.decodeParam('user_id', UserId.decode), H.mapLeft(() => InvalidArguments));
...
//   H.orElse(badRequest)
// );
const hello = pipe(H.status(H.Status.OK), // writes the response status
H.ichain(() => H.closeHeaders()), // tells hyper-ts that we're done with the headers
